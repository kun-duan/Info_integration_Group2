...
def ajax_governor_submit():
    ...
    if c_id != "":
        tub_val = [c_id, p_id]
        data_query.insert_adopt(tub_val)
...
@app.route('/yxy1', methods=['POST', 'GET'])
def method1():
    # 资金流入登记
    ...
    don = data1['don']
    don_id = data1['don_id']
    don_amount = data1['don_amount']
    don_time = data1['don_time']
    print(don)
...
@app.route('/yxy2', methods=['POST', 'GET'])
def method2():
    # 资金流出登记
    # 获取输入的姓名，身份证号码，获助金额，获助时间以及资金去向
    data3 = json.loads(request.values.get('data3'))
    rec_name = data3['rec_name']
    rec_id = data3['rec_id']
    rec_amount = data3['rec_amount']
    rec_time = data3['rec_time']
    rec_go = data3['rec_go']
    if len(rec_name) != 0:  # 判断是否输入
        ...
        db.commit()
        cur.close()
        pro = jsonify({"pro": "资金流出登记与儿童援助情况更新成功"})
    else:
        print("未填入流出资金")
    return pro


@app.route('/yxy3', methods=['POST', 'GET'])
def method3():
    # 儿童资格查询部分
    # 获得输入的儿童身份证号码
    data2 = json.loads(request.values.get('data2'))
