...
def insert_adopt(tub_val):
    conn = pymysql.connect(host="localhost", port=3306, user="root", passwd="dqy5240138", db="care")
    cursor = conn.cursor()
    cursor.execute(
        'insert into children (c_id,c_adopt) values (%s,%s)  ON DUPLICATE KEY UPDATE c_adopt=VALUES(c_adopt), c_id=VALUES(c_id)',[tub_val[0], tub_val[1]])
    conn.commit()
    cursor.close()